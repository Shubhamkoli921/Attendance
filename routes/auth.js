...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(401).json({ message: 'Invalid username or password',status:res.statusCode });
    }
    ...
    res.status(200).json({ token: token,user:user , status:res.statusCode , message:"Login successfully"});
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal server error',status:res.statusCode });
  }
});
