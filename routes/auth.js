...
router.post("/login", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Internal server error", status: res.statusCode });
  }
});
  ...
  try {
    ...
    if (user.role !== "student") {
      return res.status(403).json({
        message: "Only students are allowed to submit additional details",
      });
    }
    ...
    if (user.additionalDetailsSubmitted) {
      return res
        .status(400)
        .json({ message: "Additional details already submitted" });
    }
